...
namespace Beetroot.RecruitingBot.Scrapers
{
    public class Scraper : IScraper
    {
        ...
        private const string FindCitiesUrl = "{0}/autocomplete/city?term={1}";
        ...
        public async Task<List<SingleCity>> GetCitiesAsync(string cityName)
        {
            var url = string.Format(FindCitiesUrl, _settings.Host, cityName);
            var result = await GetDeserializedAsync<List<SingleCity>>(url);
            return result;
        }
    }
}